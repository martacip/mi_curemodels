...
surv_uncured_func = function(df, beta_W, beta_X, beta_Z){
  df$Surv = exp(-(df$H0*exp(beta_W*df$W + beta_X*df$X + beta_Z*df$Z)))
  return(df$Surv)
}
...
H0_func = function(df, beta){
  ...
  for(i in 1:length(df$Y)){
    ...
    for(tj in event_times[event_times<=df$Y[i]]){
      ...
      Rj=df[df$Y>=tj,]
      r = Rj$Q * exp((cbind(Rj$W,Rj$X,Rj$Z))%*%beta)
      denominator = sum(r)
...
cure_func = function(df){
  pi = glm(G ~ W + X + Z, family = binomial(link='logit'), data = df)
...
cox_uncured_func = function(df){
  ...
  coxph_model = coxph(surv_object ~ W + X + Z, data = df_uncured)
...
lin_pred_func = function(df, cure_param, cox_param){
  ...
  alpha_Z = alpha_star$alpha_Z
  
  beta_W = beta_star$beta_W
  beta_X = beta_star$beta_X
  beta_Z = beta_star$beta_Z
  
  df$lp_cure = alpha_X0 + df$W*alpha_W + df$X*alpha_X + df$Z*alpha_Z
  df$lp_surv = df$W*beta_W + df$X*beta_X + df$Z*beta_Z
...
imputation_exact_func = function(df,missing_indices,alpha_X0,alpha_W,alpha_X,alpha_Z,
                                 beta_W,beta_X,beta_Z,first_iter){
  ...
  if (W_distribution=="Bernoulli"){
    ...
    r = (theta_X0 + df$X*theta_X + df$Z*theta_Z) + #mu_i
      ...
      (df$G*df$H0*(exp(beta_W)-1)*exp(df$X*beta_X+df$Z*beta_Z)) + #second term
      (alpha_W*df$G) + #third term
      log(1 + exp(alpha_X0 + df$X*alpha_X + df$Z*alpha_Z)) - #fourth term
      log(1 + exp(alpha_X0 + df$W*alpha_W + df$X*alpha_X + df$Z*alpha_Z))
    R = 1/(1+exp(-r))
    ...
    target = function(W, X, Z, Y, status, G, alpha_X0, alpha_W, alpha_X, alpha_Z, beta_W, beta_X, beta_Z, H0, mu, sigma){
      kernel = ((exp(alpha_X0+X*alpha_X+W*alpha_W+Z*alpha_Z) / (1 + exp(alpha_X0+X*alpha_X+W*alpha_W+Z*alpha_Z)))^(G)) *
        ((exp((Z*beta_Z+W*beta_W+X*beta_X)*status) * exp(-H0 * exp(Z*beta_Z+W*beta_W+X*beta_X)))^(G)) *
        ((1 / (1 + exp(alpha_X0 + X*alpha_X + W*alpha_W + Z*alpha_Z)))^(1 - G)) *
        (exp((-(W - mu)^2) / (2*sigma^2)))
      return(kernel)
    }
    
    metropolis_step = function(W, X, Z, Y, status, G, alpha_X0, alpha_W, alpha_X, alpha_Z, beta_W, beta_X, beta_Z, H0, mu, sigma, sd){ 
      #the sd of the proposal distribution is a tuning parameter
      proposed_W = rnorm(1, mean = W, sd = sd)
      accept_prob = min(1, target(proposed_W, X, Z, Y, status, G, alpha_X0, alpha_W, alpha_X, alpha_Z, beta_W, beta_X, beta_Z, H0, mu, sigma) / target(W, X, Z, Y, status, G, alpha_X0, alpha_W, alpha_X, alpha_Z, beta_W, beta_X, beta_Z, H0, mu, sigma))
      u = runif(1)
      if (u <= accept_prob){
        value = proposed_W
        accepted = TRUE
    ...
    metropolis_sampler = function(initial_value, X, Z, Y, status, G, alpha_X0, alpha_W, alpha_X, alpha_Z, beta_W, beta_X, beta_Z, H0, mu, sigma, n=500, sd = 1, burnin = 0, lag = 1){
      
      results = list()
      current_state = initial_value
      for (i in 1:burnin) {
        out = metropolis_step(current_state, X, Z, Y, status, G, alpha_X0, alpha_W, alpha_X, alpha_Z, beta_W, beta_X, beta_Z, H0, mu, sigma, sd)
    ...
    out = metropolis_sampler(initial_value = df$W, df$X, df$Z, df$Y, df$status, df$G, alpha_X0, alpha_W, alpha_X, alpha_Z, beta_W, beta_X, beta_Z, df$H0, mu, sigma, burnin = 1000, lag = 100)
...
imputation_approx_func = function(df, missing_indices, k){
  ...
  df_imp = df_imp[ , c("W", "X", "Z", "G", "G_status", "G_H0")]
...
mi_smcure_func = function(data_list){
  ...
  for(j in 1:length(data_list)){
    ...
    log = capture.output({cm_mi = cureph(Surv.cure(Y,status) ~ W + X + Z, formula2 = ~ W + X + Z,
                                         data = df)
    #standard error
